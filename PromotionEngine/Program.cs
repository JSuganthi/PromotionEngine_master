...
public class PromotionCalculate{
  // Calculating the  total price of the sales invoice after applying the promotions.
   ...
   // Apply each promotion on the sales invoice and get the toatl offervalue.     
    ...
    // Calculate the total price for non promotion items, i.e left over items price after aplying promotion
          ...
          if(SalesInvoice.ContainsKey(key)){
           ...
           var key2=promotions.productOffer.ElementAt(1).Key;; 
             if(SalesInvoice.ContainsKey(key1) && SalesInvoice.ContainsKey(key2)){
               ...
               if(offerscount1<=offerscount2) // consider combo offer with least offer counts.
               {                
